...
func makeBody(value reflect.Value, params fieldParameters) (e encoder, err error) {
	...
	case reflect.Struct:
		...
		default:
			...
			for i := 0; i < n1; i++ {
				m[i], err = makeField(
					v.Field(i+startingField),
					parseFieldParameters(t.Field(i+startingField).Tag.Get("asn1")),
				)
				if err != nil {
					return nil, err
				}
...
func makeField(v reflect.Value, params fieldParameters) (e encoder, err error) {
	...
	if v.Type() == rawValueType {
		...
		t.tag = bytesEncoder(
			appendTagAndLength(t.scratch[:0], tagAndLength{rv.Class, rv.Tag, len(rv.Bytes), rv.IsCompound}),
		)
	...
	if params.tag != nil {
		...
		if params.explicit {
			t.tag = bytesEncoder(
				appendTagAndLength(t.scratch[:0], tagAndLength{ClassUniversal, tag, bodyLen, isCompound}),
			)
